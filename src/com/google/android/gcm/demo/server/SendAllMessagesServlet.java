    ...
    if (devices.isEmpty()) {// レジストレーションIDが一つも登録されていなかった場合の処理
      ...
      if (devices.size() == 1) {
        ...
        for (String device : devices) {// 拡張for文内でタスクキューを次々と実行（予約）していきます。
          ...
          if (partialSize == Datastore.MULTICAST_SIZE || counter == total) {// 上限（1000件）に達した、もしくはカウンターが全てのデバイス登録数と一致したかを確認します。
            String multicastKey = Datastore.createMulticast(partialDevices);// エンコードされたキー（プライマリーキー）を取得します。
